...
fn pop_all_sorted<K: Ord, V>(heap: &mut SlotHeap<K, V>) -> Vec<(K, V)> {
...
fn insert_and_pop_order() {
    ...
    let result = pop_all_sorted(&mut heap);
...
fn random_ops_insert_pop_remove() {
    ...
    popped_keys.extend(final_popped);
...
fn get_by_id() {
    ...
    for (i, &id) in ids.iter().enumerate() {
        assert_eq!(heap.get(id), Some(&(i as i32 * 2, ())));
        assert_eq!(heap.get_key(id), Some(&(i as i32 * 2)));
    }
    assert_eq!(heap.get(99999), None);
    assert_eq!(heap.get_key(99999), None);
}
...
fn get_value_mut() {
    ...
    for (i, &id) in ids.iter().enumerate() {
        assert_eq!(heap.get_value(id), Some(&(i as u64 * 10)));
    }
}
...
fn peek_value_mut() {
    ...
    *heap.peek_value_mut().unwrap() = 999;
    assert_eq!(heap.peek_key(), Some(&0));
    assert_eq!(heap.peek_value(), Some(&999));
}
...
fn get_key_mut_decrease() {
    ...
    assert_eq!(heap.peek_key(), Some(&-1));
    let result = pop_all_sorted(&mut heap);
    assert!(is_non_decreasing(&result));
}
